    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && preg_match('/^\/api\/hosts\/\d+\/stacks\/([a-zA-Z0-9_-]+)\/spec$/', $request_uri_path, $spec_matches)) {
        ...
        if ($is_swarm_manager) {
            ...
            if (empty($stack_services_spec)) {
                throw new Exception('No services found for this stack.');
            }
            $output_spec = ['services' => $stack_services_spec];
        } else {
            // Non-swarm node: build spec from containers
            $all_containers = $dockerClient->listContainers();
            $stack_containers_spec = [];
            foreach ($all_containers as $container) {
                $compose_project = $container['Labels']['com.docker.compose.project'] ?? null;
                if ($compose_project === $stack_name) {
                    $compose_service_name = $container['Labels']['com.docker.compose.service'] ?? 'unknown_service';
                    // We can't get the original spec, so we show the container's config as a representation
                    $stack_containers_spec[$compose_service_name] = [
                        'image' => $container['Image'],
                        'container_name' => $container['Names'][0] ?? 'N/A',
            ...
            if (empty($stack_containers_spec)) {
                throw new Exception('No containers found for this compose project.');
            }
            $output_spec = ['containers' => $stack_containers_spec];
        }

        $yaml_output = Spyc::YAMLDump($output_spec, 2, 0);
